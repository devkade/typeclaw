...
| Platform                                    | Path                                                                | `readOnly` | TCC     | Notes                                                                                                                                                                                                                                                                                               |
| ------------------------------------------- | ------------------------------------------------------------------- | ---------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| macOS 13+ (Ventura, Sonoma, Sequoia, Tahoe) | `~/Library/Mail/V10`                                                | `true`     | **FDA** | The `V10` directory holds per-account `.mbox`-like dirs containing `.emlx` files (one per message). The format is parseable as RFC822 + a small Apple-specific binary plist trailer. The version bumped to `V10` in Ventura and has stayed there through Tahoe.                                     |
| macOS 12 (Monterey)                         | `~/Library/Mail/V9`                                                 | `true`     | **FDA** | Same shape, previous version dir.                                                                                                                                                                                                                                                                   |
...
| WSL — Outlook (Win32)                       | `/mnt/c/Users/<name>/AppData/Local/Microsoft/Outlook/<mailbox>.pst` | `true`     | —       | Replace `<mailbox>` with a real PST filename, or mount the `.../Microsoft/Outlook/` dir and point at the file — mounts don't expand globs, so a literal `*.pst` won't resolve. PST is a proprietary binary format; the agent needs `libpff` or similar to read it. Not as plug-and-play as `.emlx`. |
...
| ---------------------------------------- | --------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Shell history                            | `~/.zsh_history` or `~/.bash_history`                           | `true`     | "What command did I run yesterday?" Mount the specific history file — its parent is the home directory, which the anti-patterns forbid mounting.                                                                                                                                                                          |
...
- **macOS path versioning will drift.** Apple has moved `~/Library/Mail/V<n>`, Voice Memos, and other paths across macOS versions. The paths in this file are good for macOS 11–26 as of this writing; if a future macOS reorganization breaks one, the symptom is `ls mounts/<name>` returning empty after a successful mount. Update this file, don't paper over it.
