...
- `network` — the separate container-stage egress firewall. `blockInternal` defaults on; `allow` accepts explicit IPv4 addresses/CIDRs for all-protocol firewall exceptions. It does not bypass `web_fetch` or URL-backed `look_at` SSRF validation. **Restart-required.**
...
- **Restart-required fields** (`port`, `mounts`, `plugins`, `portForward`, `network`, `docker.file`, `git.ignore`) are reported as "reload landed but change won't apply until restart". The diff returns success; the runtime still has the old value in memory. Tell the user explicitly which one they're hitting. `docker.file` additionally requires an image rebuild — that happens automatically on the next `typeclaw start`, no extra flag needed. `git.ignore` refreshes the managed `.gitignore` and auto-commits it on the next `typeclaw start` if content changed.
...
| `network`                        | no       | object           | Container-stage egress firewall: `{ blockInternal, autoAllowResolvers, allow }`. Defaults to internal blocking on, resolver carve-outs on, and no IPv4 exceptions. `allow` is independent from safe-HTTP SSRF exceptions. **Restart-required.** See **Network and model HTTP internal destinations** below.                                                                                                                                                                                                                                                                                                              |
...
The runtime fills in defaults for any omitted field: `port` → `8973`, `mounts` → `[]` (no host paths exposed), `plugins` → `[]`, `channels` → `{}` (no adapters configured), `portForward` → `{ "allow": "*" }` (forward every container LISTEN port), `network` → `{ "blockInternal": true, "autoAllowResolvers": true, "allow": [] }`, `docker` → `{ "file": { "ffmpeg": false, "gh": true, "python": true, "tmux": true, "cjkFonts": "auto", "cloudflared": false, "xvfb": true, "claudeCode": false, "codexCli": false, "append": [] } }` (tmux/gh/python/xvfb pre-installed; cjkFonts auto-detected from host locale; ffmpeg, cloudflared, claudeCode, and codexCli off; no custom build steps), `git` → `{ "ignore": { "append": [] } }` (no custom ignore patterns). `typeclaw init` deliberately omits any field whose default is owned elsewhere — `mounts`, `portForward`, `network`, `docker`, and `git` default via `configSchema`, and the bundled memory plugin owns its own `memory` defaults — so the scaffolded file stays minimal and the user sees only fields they actually need to think about. Add a `memory` block (a **plugin config block** owned by the bundled memory plugin) only when overriding its defaults; see the `typeclaw-memory` skill for the schema.
...
## Network and model HTTP internal destinations

These are two independent gates:

1. **Container firewall:** `typeclaw.json#network` defaults to `{ "blockInternal": true, "autoAllowResolvers": true, "allow": [] }`. `network.allow` accepts bare IPv4 addresses or IPv4 CIDRs and permits matching traffic across all ports/protocols. It is restart-required and is not an exception to application-layer SSRF checks.
2. **Safe HTTP transport:** internal exceptions for `web_fetch` and URL-backed `look_at` are operator-owned boot settings in raw-masked `.env`, never `typeclaw.json`, role permissions, or `acknowledgeGuards`:
   - `TYPECLAW_MODEL_HTTP_ALLOW_INTERNAL_HOSTS=service.corp,printer.local` — exact case-insensitive, IDNA-normalized hostnames; no wildcards, suffix patterns, URLs, or IP literals.
   - `TYPECLAW_MODEL_HTTP_ALLOW_INTERNAL_CIDRS=10.20.0.0/16,fd12:3456::/48` — IPv4/IPv6 CIDRs with explicit prefixes.
...
3. **Write the whole file back** with the `write` tool. Always pretty-printed (2-space indent), trailing newline, fields in stable order: `$schema` first, then alphabetical for the rest (`alias`, `channels`, `docker`, `git`, `model`, `mounts`, `network`, `plugins`, `port`, `portForward`, then any plugin config blocks like `memory`).
...
6. **Tell the user the right next step.** Match the field's effect class:
   ...
   - `port`, `mounts`, `network`, `plugins`, `portForward` → "Restart-required. Run `typeclaw restart` (host stage) to pick up the change."
...
- If `network` is set: `blockInternal` and `autoAllowResolvers` are booleans; `allow` is an array of bare IPv4 addresses or IPv4 CIDRs only
...
- No unknown top-level keys you invented — keys outside the documented core fields are interpreted as **plugin config blocks** and only do something if a plugin owns them. Inventing one means the user thinks it took effect and it did not.
