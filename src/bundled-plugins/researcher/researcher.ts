...
If none of the listed skills fit the question, load \`general\`. Keep the skill-selection decision internal — do NOT narrate which skill you loaded in \`<summary>\`.`,
  ...
  return {
    ...
    // No `requiresSpecificPermission`: the researcher's only write goes through
    // the `write_report` tool, which enforces "one report file under
    // workspace/public" in code. Any caller that can spawn a subagent can spawn
    // the researcher.
