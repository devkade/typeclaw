...
- **`.env` is the operator's expose-to-model-bash surface, not a hiding place.** Every var an operator declares in `.env` (with a non-empty value) is re-introduced into the bwrap per-tool sandbox via `inherit`, so it reaches model-driven bash by design — **including `GH_TOKEN`/`GITHUB_TOKEN`** (an operator with no GitHub channel puts a PAT there so plain `gh`/`git`/`curl` work). `resolveExposableEnvNames` (`src/sandbox/env-exposure.ts`) only withholds names that would subvert the sandbox mechanism or claim a host/container-injected runtime token: the sandbox-owned `PATH`/`HOME`/`BUN_*`/`LANG`, process/loader-hijack vars (`LD_PRELOAD`, `NODE_OPTIONS`, `BASH_ENV`, `SHELLOPTS`, the `BASH_FUNC_`/`GIT_CONFIG` prefixes, `SSH_AUTH_SOCK`, `KUBECONFIG`, …), and the `TYPECLAW_*` tokens. Do NOT re-add `GH_TOKEN`/`GITHUB_TOKEN` to that withhold set — the `github-cli-auth` broker's narrower per-repo overlay is layered on top (it's deduplicated against the `.env` inherit and its value wins for `gh`), not a substitute for it. Credentials that must stay hidden from the agent belong in `secrets.json`, which is never inherited into bash.
