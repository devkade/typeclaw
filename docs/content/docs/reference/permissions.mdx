...
| `subagent.spawn.operator` | `owner`/`trusted`          | per-subagent grant admitting `operator` on its own; `operator` is also covered by the generic `subagent.spawn`, so this is only needed to admit it _without_ the generic grant |
...
| `ssrf`                 | `security.bypass.ssrf`                 | heuristic pre-tool checks for internal `web_fetch` targets; model acknowledgement is not accepted, and the safe transport independently enforces the operator's boot-only environment policy plus a non-overridable metadata/control-plane floor |
...
The non-overridable SSRF floor also recognizes metadata/control-plane IPv4 addresses carried by standardized IPv4-compatible, mapped, translatable, well-known NAT64, and 6to4 IPv6 forms. It blocks the full RFC 8215 local-use `64:ff9b:1::/48` range because the standard is technology-agnostic: this does not mean every member maps metadata, but it makes safe metadata discrimination impossible without assuming an IPv4 field location. Coverage is intentionally bounded to those standard ranges; TypeClaw does not infer arbitrary operator-specific RFC 6052 prefixes.
