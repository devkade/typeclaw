...
Every production model-driven bash call runs in bwrap with the inherited process environment cleared, including for `trusted` and `owner`. TypeClaw adds back the sandbox-owned allowlist plus **every var an operator declared in `.env`** — `.env` is the deliberate expose-to-the-agent surface, so anything you put there (including `GH_TOKEN`/`GITHUB_TOKEN`, e.g. so plain `gh`/`git`/`curl` work in agents with no GitHub channel) reaches model bash. Credentials you do not want the agent to read must live in `secrets.json`, which is never inherited into bash. Because a declared `.env` var is already exposed this way, a `.env` accidentally committed to the agent repo's Git history does **not** trip the [Git metadata preflight](/docs/internals/sandbox) that otherwise disables model bash on canonical secrets — a historical `.env` discloses nothing the live environment does not; `secrets.json`/`auth.json` in history still block. On top of that, runtime-owned credential overlays, such as a per-repo GitHub token minted by the `github-cli-auth` broker, are injected directly into one narrowly parsed subprocess; they do not make `.env`, `secrets.json`, or historical root `auth.json` readable. GitHub token-display and auth-management commands, including `gh auth status -t` and `--show-token`, are blocked before injection.
