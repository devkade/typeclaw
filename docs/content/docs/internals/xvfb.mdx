...
description: The headed-via-Xvfb decision, NET_ADMIN drop, ephemeral runtime HOME, agent-browser headed-mode wrapper
icon: Monitor
---
...
## Ephemeral runtime `$HOME`

The trusted in-process runtime uses `/home/agent`, which lives only in the container writable layer and disappears when `typeclaw stop` removes the container. The entrypoint's root bootstrap creates it unconditionally, copies image-baked `~/.claude.json`, `~/.claude/settings.json`, and `~/.codex/hooks.json` from `/root`, then chowns only that subtree when a dynamic host UID/GID is available. Both privilege-drop handoffs and both root-compatible fallbacks exec the runtime with `HOME=/home/agent`; no UID/GID is hardcoded.

CLI OAuth files are reconstructed there from `secrets.json` at boot. Provider OAuth refresh is awaited before the Codex and Claude exporters, so a fresh container receives the refreshed token rather than a stale pre-refresh copy. TypeClaw does not persist, migrate, or delete the former `/agent/.typeclaw/home/` overlay. The path stays gitignored and masked as a canonical legacy secret location for upgraded agents that still have orphaned credentials.

This is independent of model-driven bash: bwrap builds a narrower filesystem view that does not bind `/home`, resets sandbox `HOME` to `/tmp`, and masks canonical agent secret paths. Non-bash model tools deny both `/home/agent` CLI profiles and the legacy overlay explicitly.
